...
1. In `dns/values.yaml`, set `configmap.hosts[].ip` to the host machine's IP:
...
2. In `kind-configs.yaml`, set the `listenAddress` values under `HTTP` and `HTTPS` to the host machine's IP.
3. In the same file, set the `listenAddress` values under `DNS - UDP` and `DNS - TCP` to the DNS server's IP.

## 2. Create the Secrets

Add a `secrets.yaml` file in the `meguurl/templates` directory. The file should contain the following:
...
```bash
kind create cluster -n <cluster-name> --config kind-configs.yaml
```
...
helm install trust-manager trust-manager \
    --namespace cert-manager

helm install cert-distributer cert-distributer \
    --namespace cert-manager \
...
```bash
helm install cert-downloader cert-downloader \
